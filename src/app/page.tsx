...
import { adviceWorkerVideoURL, gridFaqList, keyPointsList, lowBackPainVideoURL, postOpExpectationVideoURL, researchEvidenceList, riskVideoURL, spinalFusionVideoURL } from "./constants/homepage.constants";
    ...
    <Box>
            ...
            <Box className="space-y-6" style={{opacity: 1, transform: 'none'}}>
              <Box className="bg-white p-6 rounded-xl shadow-md">
                ...
                <DataList.Root orientation="horizontal" className="space-y-4">
                  {keyPointsList && keyPointsList.map((items, key) => (
                    <DataList.Item key={key} align="center" className="flex items-start gap-3">
                      <DataList.Label>
                        <INFO_ICON width="24" height="24"/>
                      </DataList.Label>
                      <DataList.Value className="text-gray-600">{items}</DataList.Value>
                    </DataList.Item>
                  ))}
                </DataList.Root>
              </Box>
      ...
      <SectionBlockComponent Id="faq" RootClass="relative py-12 bg-white" ChildClass="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <GridBlockComponent GridClass="md:grid-cols-2 gap-8 grid-cols-1">
            {gridFaqList && gridFaqList.map((item, key) => (
              <Box 
                key={key} 
              ...
              >
                {item.icon}
                <Heading as="h3" className="text-xl font-semibold text-primary mb-2">{item.title}</Heading>
          ...
          <GridBlockComponent GridClass="md:grid-cols-1 gap-8 p-8 items-center">
            <Box as="div" className="flex items-center justify-center" style={{opacity: 1, transform: 'none'}}>
      ...
      <SectionBlockComponent Id="chronic" RootClass="bg-[#FAFAFA] py-20" ChildClass="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <GridBlockComponent GridClass="md:grid-cols-2 gap-8 lg:gap-12 items-center">
            <Box className="space-y-6">
              <Heading as="h2" className="text-3xl sm:text-4xl font-bold text-primary">
                There's more to chronic low back pain than most realise...
              </Heading>
              <Box className="h-1 w-20 bg-secondary rounded-full"></Box>
            </Box>
            <Box as="div" className="aspect-video w-full rounded-xl overflow-hidden bg-gray-100">
              <VideoPlayerComponent url={lowBackPainVideoURL} />
            </Box>
        </GridBlockComponent>
      </SectionBlockComponent>

      <SectionBlockComponent RootClass="bg-[#F0F0F0] py-16" ChildClass="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Box as="div" className="space-y-12" style={{opacity: 1, transform: 'none'}}>
            <Box>
              <Heading as="h2" className="text-3xl font-bold text-primary mb-8">Research Evidence</Heading>
              <Box as="div" className="space-y-4">
                {researchEvidenceList && researchEvidenceList.map((item, key) => (
                  <Link key={key} rel="noopener noreferrer" target="_blank" className="flex items-center gap-2 text-secondary hover:text-secondary/80 transition-colors" style={{opacity: 1, transform: 'none'}}
                  href={item.url}>
